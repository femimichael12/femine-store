...
import { ArrowLeft, Heart, Sun, Moon } from 'lucide-react';
...
export default function WishlistPage({ onExit, theme, toggleTheme, user }: WishlistPageProps) {
      ...
      <main className="w-full max-w-4xl flex-grow flex flex-col justify-center relative z-10 py-10">
        ...
        >
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-muted/15 pb-6">
            <div>
              <h1 className="text-3xl font-serif font-bold">My Saved Favorites</h1>
              <p className="text-xs text-muted-foreground mt-1">
                {user ? `Curated wishlist for ${user.email}` : 'Sign in to save your favorite luxury items'}
              </p>
            </div>
          ...
          <div className="py-12 flex flex-col items-center justify-center text-center space-y-4">
            <div className="w-16 h-16 rounded-full bg-brand-coral/10 text-brand-coral flex items-center justify-center border border-brand-coral/20">
              <Heart className="w-8 h-8 fill-current text-brand-coral" />
            </div>
            <div className="space-y-1">
              <h3 className="text-xl font-serif font-bold">Your Wishlist is Empty</h3>
              <p className="text-xs text-muted-foreground max-w-sm mx-auto">
                Click the heart icon on any product card while browsing to add items to your wishlist.
              </p>
            </div>
            <Button 
              onClick={onExit}
